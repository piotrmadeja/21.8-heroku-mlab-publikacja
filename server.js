...
const findSpecificRecord = function() {
    ...
    return User.find({ username: 'Kenny_the_boy2' }, function(err, res) {
        if (err) throw err;
...
const updadeUserPassword = function() {
    ...
    return User.findOne({ username: 'Kenny_the_boy2' })
        .then(function(user) {
...
const updateUsername = function() {
    // update username
    return User.findOneAndUpdate({ username: 'Benny_the_boy2' }, { username: 'Benny_the_man' }, { new: true }, function(err, user) {
        if (err) throw err;
...
const findMarkAndDelete = function() {
    ...
    return User.findOne({ username: 'Mark_the_boy2' })
        .then(function(user) {
